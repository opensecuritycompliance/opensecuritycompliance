		...
		if err != nil {
			...
			for _, rule := range ruleSet.Rules {
				...
				for _, task := range taskInfos {
					...
					installPythonDependenciesWithRequirementsTxtFile(taskFolderName, taskFolderName)
					installPythonDependenciesWithRequirementsTxtFile(filepath.Join(taskFolderName, "applicationtypes"), taskFolderName)
...
func installPythonDependenciesWithRequirementsTxtFile(reqDir string, venvBaseDir string) error {
	...
	venvDir, err := filepath.Abs(filepath.Join(venvBaseDir, ".venv"))
	if err != nil {
		return fmt.Errorf("failed to get absolute venv path: %w", err)
	}
	fmt.Printf("Installing requirements.txt | reqDir=%s | venv=%s\n", reqDir, venvDir)

	if _, err := os.Stat(venvDir); os.IsNotExist(err) {
		cmd := exec.Command("uv", "venv", venvDir, "--system-site-packages")
		if out, err := cmd.CombinedOutput(); err != nil {
			fmt.Println("uv package init error:", err, "\noutput:", string(out))
			return fmt.Errorf("failed to create venv: %w, output: %s", err, string(out))
		}
	}

	pythonPath := filepath.Join(venvDir, "bin", "python")
	cmd := exec.Command("uv", "pip", "install", "-r", "requirements.txt", "--python", pythonPath)
	cmd.Dir = reqDir
	...
	fmt.Println("installation output:", string(out))
	return nil
}
...
func taskExecutor(taskName string) error {
	...
	if isPythonFlow {
		venvDir := filepath.Join(taskDir, ".venv")
		pythonPath := filepath.Join(venvDir, "bin", "python")

		if _, err := os.Stat(pythonPath); err != nil {
			return fmt.Errorf("venv python not found for task %s at %s: %w", taskName, pythonPath, err)
		}
...
spec:
  ...
  outputsMeta__:
    - name: OutputFile
      dataType: FILE
      description: File that contains the processed output or results generated by the rule execution
      repeated: false
      allowedValues: []
    ...
    - name: LogFile
      dataType: FILE
      description: File that contains information about errors that have occurred while executing rule
      repeated: false
      allowedValues: []
      defaultValue: <<MINIO_FILE_PATH>>
      showField: true
