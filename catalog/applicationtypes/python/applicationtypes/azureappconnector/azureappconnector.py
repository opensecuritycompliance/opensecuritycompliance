...
class AzureAppConnector:
    ...
    storage_account_api_scope = "https://storage.azure.com"
    domain_name = ''
    ...
    def get_repository_list_using_registry(self, registry_name):
        try:
            ...
            while True:
                repo_list_url = f"https://{registry_name}.azurecr.io/acr/v1/_catalog?last={last_repo_name}&n=100"
                ...
                last_repo_name = repositories[-1]

            if all_repositories is not None:
                return all_repositories, None
            else:
    ...
    def delete_user_from_group(self , user_id , group_id ):
        ...
        if not response.ok and not response.status_code == 204:
    ...
    def get_user_by_group_id(self , group_id):
    ...
    def get_user_groups_id(self, user_id):
        try:
            token, err = self.get_access_token(scope='https://graph.microsoft.com/.default')
            if err:
                return None, f"Error while fetching access token. {err}"
            ...
            if len(response_dict['value']) == 0:
                return ["No groups attached"], None
            res_df = pd.DataFrame(response_dict['value'])
            return res_df['id'].values.tolist(), None
        except requests.exceptions.HTTPError as e:
            logging.exception("An http exception occured while fetching group details for user - %s: %s", str(user_id), str(e))
            return None, f"{GRP_ERR_MSG}. Http exception occured. {SUPPORT_MSG}."
        except RequestException as e:
    ...
    def download_file_from_storage_account(self, storage_account_name: str, file_path: str) -> tuple[str, str]:
        auth_payload = {
            "clientID": self.user_defined_credentials.azure.client_id,
            "clientSecret": self.user_defined_credentials.azure.client_secret,
            "tenantID": self.user_defined_credentials.azure.tenant_id,
            "scope": self.storage_account_api_scope
        }

        token, auth_error = self.get_azure_access_token(auth_payload)
        if auth_error:
            return '', auth_error
        ...
        url = f'https://{storage_account_name}.blob.core.windows.net/{file_path}?action=read'
        ...
        try:
            ...
            error_code, error_message = self.extract_error_details(resp.text)
            return '', f'{error_code}: {error_message}'

        except requests.exceptions.ConnectionError as e:
            return '', f'Connection error: Unable to connect to {url}. Details: {str(e)}'
    ...
    def extract_error_details(self, xml_response: str) -> tuple[str, str]:
        try:
            ...
            error_code = root.findtext("Code", default="UnknownError")
            error_message = root.findtext(
                "Message", default="No error message provided")
