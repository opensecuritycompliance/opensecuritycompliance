...
EC2_INSTANCE_DETAILS = "AwsEc2InstanceDetails"
ELASTIC_FILE_SYSTEM = "AwsEFS"
ELASTIC_KUBERNETES_SERVICE = "AwsEKS"
...
IAM_USER_LIST_URL = (
    "https://<<Region>>.console.aws.amazon.com/iamv2/home?region=<<Region>>#/users"
)
...
class AWSRole:
    ...
    def __init__(self, access_key: str, secret_key: str, role_arn: str) -> None:
...
class LinkedApplications:
    ...
    def __init__(self, compliance_cow_app: List[compliancecow.ComplianceCow]) -> None:
    ...
    def to_dict(self) -> dict:
        result: dict = {}
        result["ComplianceCow"] = [item.to_dict() for item in self.compliance_cow_app]
        return result
...
class AWSAppConnector:
    ...
    def __init__(
        ...
        region: list = None,
        linked_applications: LinkedApplications = None,
    ) -> None:
    ...
    def from_dict(obj) -> "AWSAppConnector":
        app_url, app_port, user_defined_credentials, linked_applications = (
            "",
            "",
            None,
            None,
        ...
        return AWSAppConnector(
            app_url, app_port, user_defined_credentials, region, linked_applications
        )
    ...
    def is_empty_aws_role(self):
        ...
        return not all(
            (role_creds.access_key, role_creds.secret_key, role_creds.role_arn)
        )

    def create_aws_session_with_role(
        self,
        region=None,
        role_session_name="compliancecowsession",
        aws_service_name="sts",
    ):
        ...
        try:
            ...
            assumed_role = sts_client.assume_role(
                RoleArn=aws_role.role_arn, RoleSessionName=role_session_name
            )
            if (
                cowdictutils.is_valid_key(assumed_role, "Credentials")
                and cowdictutils.is_valid_key(
            ...
            return (
                None,
                f"Endpoint connection error: {error}. Check if AWS STS is enabled in region '{region}'.",
            )
    ...
    def get_user_login_profile(self, user_name: str):
        try:
            ...
            if error:
                return None, f"Error while creating session: {error}"
    ...
    def describe_vpcs(self):
        try:
            ...
            for region in self.region:
                ...
                try:
                    for page in paginator.paginate():
                        vpcs = page.get("Vpcs", [])
                        if len(vpcs) != 0:
                            vpcs_flat = pd.json_normalize(vpcs)
                            vpcs_df = pd.concat(
            ...
            if not vpcs_df.empty:
                vpcs_df.columns = pd.Series(vpcs_df.columns).apply(
    ...
    def describe_flow_logs(self):
        try:
            ...
            for region in self.region:
                ...
                try:
                    for page in paginator.paginate():
                        ...
                        if len(flow_logs) != 0:
                            flow_logs_flat = pd.json_normalize(flow_logs)
                            flow_logs_df = pd.concat(
                                [flow_logs_df, pd.DataFrame(flow_logs_flat)],
                                ignore_index=True,
                            )
                except botocore.exceptions.ClientError as error:
                    error_msg = error.response["Error"]["Message"]
            ...
            if not flow_logs_df.empty and "CreationTime" in flow_logs_df.columns:
                ...
                flow_logs_df.columns = pd.Series(flow_logs_df.columns).apply(
                    self.clean_column_names
                )
        ...
        try:
                ...
                try:
                    for page in paginator.paginate():
                        ...
                        if len(security_groups) != 0:
                            security_groups_flat = pd.json_normalize(security_groups)
                            security_groups_df = pd.concat(
                                [
                                    security_groups_df,
                                    pd.DataFrame(security_groups_flat),
                                ],
                                ignore_index=True,
                            )
                except botocore.exceptions.ClientError as error:
            ...
            if not security_groups_df.empty:
                security_groups_df.columns = pd.Series(
                    security_groups_df.columns
                ).apply(self.clean_column_names)
            ...
            try:
                ...
                if not cowdictutils.is_valid_array(response, "trailList"):
                    errors_list.append(
                        {
                            "Region": region,
                            "Error": "Could not get trail list from response",
                        }
                    )
                    continue
                trail_list = response["trailList"]
                trail_list_flat = pd.json_normalize(trail_list)
                ...
                if (
                    hasattr(error, "kwargs")
                    and isinstance(error.kwargs, dict)
                    and cowdictutils.is_valid_key(error.kwargs, "endpoint_url")
                ):
    ...
    def get_caller_identity(self):
        ...
        try:
            ...
            identity_details_flat = pd.json_normalize(identity_details)
            identity_details_df = pd.concat(
                [identity_details_df, pd.DataFrame(identity_details_flat)],
                ignore_index=True,
            )
                        ...
                        for key in response["Keys"]:
                            if cowdictutils.is_valid_key(key, "KeyId"):
                                ...
                                if cowdictutils.is_valid_key(
                                    ...
                                    key_rotation_details, err = (
                                        self.get_key_rotation_status(key_id, kms_client)
                                    )
                                    if err:
                                        errors_list.append(
                                    ...
                                    key_rotation_enabled = key_rotation_details[
                                        "KeyRotationEnabled"
                                    ]
                                    rotation_period_in_days = key_rotation_details[
                                        "RotationPeriodInDays"
                                    ]
                                    next_rotation_date = key_rotation_details[
                                        "NextRotationDate"
                                    ]
                                    # Calculate LastRotationDate obj
                                    last_rotation_obj = next_rotation_date - timedelta(
                                        days=rotation_period_in_days
                                    )

                                    key_details_flat["KeyRotationEnabled"] = (
                                        key_rotation_enabled
                                    )
                                    key_details_flat["KeyLastRotatedDate"] = (
                                        self.get_time_stamp(last_rotation_obj)
                                    )
                                    key_details_flat["KeyNextRotatedDate"] = (
                                        self.get_time_stamp(next_rotation_date)
                                    )
                                    kms_keys_df = pd.concat(
                                        [kms_keys_df, pd.DataFrame(key_details_flat)],
                                        ignore_index=True,
                                    )
                                else:
                                    errors_list.append(
                                        {
                                            "Region": region,
                                            "Error": "Missing field: KeyMetaData",
                                        }
                                    )
                                    continue
                            else:
                                errors_list.append(
                                    {"Region": region, "Error": "Missing field: KeyId"}
                                )
                                continue
                    else:
                        errors_list.append(
                            {"Region": region, "Error": "Missing field: Keys"}
                        )
                        continue
    ...
    def describe_instance(
        ...
        """
        Retrieves information about a specific EC2 instance across all configured AWS regions.
        This method attempts to describe an EC2 instance with the given `instance_id` in each region specified in `self.region`.
        It returns the response from the first successful region, or a list of errors encountered in each region if the instance
        could not be described.
        Args:
            instance_id (str): The ID of the EC2 instance to describe.
        Returns:
            Tuple[Optional[dict], Optional[list]]:
                - If successful, returns a tuple containing the response dictionary from the AWS EC2 `describe_instances` API and `None` for errors.
                - If unsuccessful in all regions, returns `None` and a list of error dictionaries, each containing the region and error message.
        """

        errors_list = []
        for region in self.region:
            session, error = self.create_aws_session(region=region)
            if error:
                errors_list.append({"Error": error})
                return None, errors_list
            try:
                ...
                error_message = "An error occurred while trying to fetch the response for the describe_instance operation."
                if (
                    hasattr(error, "response")
                    and isinstance(error.response, dict)
                    and cowdictutils.is_valid_key(error.response, "Error")
                ...
                error_message = (
                    "Could not connect to the describe_instance endpoint URL"
                )
                if (
                    hasattr(error, "kwargs")
                    and isinstance(error.kwargs, dict)
                    ...
                    endpoint_url = error.kwargs["endpoint_url"]
                    error_message = f'Could not connect to the endpoint URL: "{endpoint_url}" in region "{region}" while attempting to access the describe_instance service.'
                errors_list.append({"Region": region, "Error": error_message})
                continue
    ...
    def describe_cluster(
        ...
        """
        Describes an Amazon EKS cluster by its cluster ID across multiple AWS regions.
        Attempts to create an AWS session for each region specified in `self.region` and retrieves
        the cluster details using the EKS `describe_cluster` API. If the cluster is found in a region,
        the response is returned immediately. If errors occur (such as client errors or endpoint
        connection errors), they are collected and returned after all regions have been attempted.
        Args:
            cluster_id (str): The name or ID of the EKS cluster to describe.
        Returns:
            Tuple[Optional[dict], Optional[list]]:
            - If successful, returns a tuple containing the response dictionary from the AWS EKS `describe_cluster` API and `None` for errors.
            - If unsuccessful in all regions, returns `None` and a list of error dictionaries, each containing the region and error message.
        """
        ...
        for region in self.region:
            ...
            try:
                ...
                response = client.describe_cluster(name=cluster_id)
                return response, None
            except botocore.exceptions.ClientError as error:
                error_message = "An error occurred while trying to fetch the response for the describe_cluster operation."
                if (
                    hasattr(error, "response")
                    and isinstance(error.response, dict)
                    and cowdictutils.is_valid_key(error.response, "Error")
                ...
                error_message = "Could not connect to the describe_cluster endpoint URL"
                if (
                    hasattr(error, "kwargs")
                    and isinstance(error.kwargs, dict)
                    and cowdictutils.is_valid_key(error.kwargs, "endpoint_url")
                    ...
                    endpoint_url = error.kwargs["endpoint_url"]
                    error_message = f'Could not connect to the endpoint URL: "{endpoint_url}" in region "{region}" while attempting to access the describe_cluster service.'
                errors_list.append({"Region": region, "Error": error_message})
                continue
    ...
    def get_key_rotation_status(self, key_id, kms_client):
        try:
            ...
            if (
                cowdictutils.is_valid_key(response, "KeyRotationEnabled")
                and cowdictutils.is_valid_key(response, "RotationPeriodInDays")
                and cowdictutils.is_valid_key(response, "NextRotationDate")
            ):
                return response, None
            else:
    ...
    def describe_metric_filters(self):
            ...
            try:
                for page in paginator.paginate():
                    if cowdictutils.is_valid_array(page, "metricFilters"):
                        ...
                        metric_filters_flat = pd.json_normalize(metric_filters)
                        metric_filters_df = pd.concat(
                            [metric_filters_df, pd.DataFrame(metric_filters_flat)],
                            ignore_index=True,
                        )
            except botocore.exceptions.ClientError as error:
        ...
        if not metric_filters_df.empty and "creationTime" in metric_filters_df.columns:
            ...
            metric_filters_df["creationTime"] = self.convert_timestamp(
                metric_filters_df["creationTime"], unit="ms"
            )

            if "metricTransformations" in metric_filters_df.columns:
                # metricTransformations: fixed number of 1 item
                try:
                    metric_transformations = pd.DataFrame(
                        metric_filters_df["metricTransformations"].tolist()[0]
                    )
                    metric_filters_df = pd.concat(
                        [metric_filters_df, metric_transformations], axis=1
                    )
                    metric_filters_df.drop(
                        columns="metricTransformations", inplace=True
                    )
                except (IndexError, ValueError, TypeError) as error:
                    errors_list.append({"Error": f"{error}"})
                    return None, errors_list

            metric_filters_df.rename(
                columns={
            ...
            try:
                for page in paginator.paginate():
                    if cowdictutils.is_valid_array(page, "DBInstances"):
                        ...
                        db_instances_flat = pd.json_normalize(db_instances)
    ...
    def describe_metric_alarms(self):
            ...
            try:
                for page in paginator.paginate(AlarmTypes=["MetricAlarm"]):
                    if cowdictutils.is_valid_array(page, "MetricAlarms"):
                        ...
                        metric_alarms_flat = pd.json_normalize(metric_alarms)
                        metric_alarms_df = pd.concat(
                            [metric_alarms_df, pd.DataFrame(metric_alarms_flat)],
                            ignore_index=True,
                        )
            except botocore.exceptions.ClientError as error:
        ...
        if not metric_alarms_df.empty:
            if "AlarmConfigurationUpdatedTimestamp" in metric_alarms_df.columns:
                metric_alarms_df["AlarmConfigurationUpdatedTimestamp"] = (
                    self.convert_timestamp(
                        metric_alarms_df["AlarmConfigurationUpdatedTimestamp"]
                    )
                )
            if "StateTransitionedTimestamp" in metric_alarms_df.columns:
                metric_alarms_df["StateTransitionedTimestamp"] = self.convert_timestamp(
                    metric_alarms_df["StateTransitionedTimestamp"]
                )
            if "StateUpdatedTimestamp" in metric_alarms_df.columns:
                metric_alarms_df["StateUpdatedTimestamp"] = self.convert_timestamp(
                    metric_alarms_df["StateUpdatedTimestamp"]
                )
        ...
        try:
            ...
            if session:
                ...
                if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
                    return (
                        None,
                        f"Failed to upload file to S3. Status code: {response['ResponseMetadata']['HTTPStatusCode']}",
                    )
            ...
            if (
                ...
                if error.response["Error"]["Code"] == "403":
                    return (
                        None,
                        f"User doesn't have write access for the given bucket {bucket_name}. Please check permissions.",
                    )
                else:
                    error_message = f"{error.response['Error']['Code']}-{error.response['Error']['Message']}"
            return None, error_message
        except botocore.exceptions.EndpointConnectionError as error:
        ...
        try:
            ...
            if (
                ...
                if error.response["Error"]["Code"] == "NoSuchBucket":
                    return (
                        None,
                        f"Bucket name: {bucket_name} is not found. Please provide a valid bucketname.",
                    )
    ...
    def convert_timestamp(self, data, unit=None):
        ...
        if isinstance(data, str):
            ...
            if unit:
                return pd.to_datetime(data, unit=unit).dt.strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                )
            else:
    ...
    def get_resource_url(self, resource_info: dict):
        ...
        if cowdictutils.is_valid_key(resource_info, RESOURCE_TYPE):
            ...
            if cowdictutils.is_valid_key(resource_url_dict, resource_type):
                url, error = self.modify_url(
                    resource_url_dict[resource_type], resource_info
                )
                if error:
                    return None, error
                return url, None
    ...
    def modify_url(self, url, resource_info):
        ...
        for placeholder in placeholders:
            if cowdictutils.is_valid_key(placeholder_map, placeholder):
                ...
                if field:
                    if (
                        cowdictutils.is_valid_key(resource_info, field)
                        and resource_info[field] != ""
                    ):
                        url = url.replace(placeholder, resource_info[field])
                    else:
                        return (
                            None,
                            f"Required field {field} not found or empty in input dict",
                        )
        return url, None
        ...
        for key, value in json_obj.items():
            ...
            if isinstance(value, dict) and level != 1:
                flattened_dict.update(
                    self.flatten_json(value, new_key, level=level + 1)
                )
            else:
    ...
    def get_file_uris_from_s3_objects(self, objects, bucket_name):
        ...
        try:
            ...
            return (
                None,
                f"Exception occured while generating s3 uri(s) for provided object(s). {str(e)}",
            )
    ...
    def get_all_file_content_from_s3_bucket(self, bucket_name, folder_path=None):
        ...
        try:
            ...
            return (
                None,
                f"Failed to fetch file content from AWS bucket {bucket_name}. Please contact support and review logs for further details.",
            )
        except botocore.exceptions.ClientError as error:
            ...
            if (
                ...
                logging.exception(
                    f"EndpointConnectionError exception occurred: {error}"
                )
            return None, error_message
        except Exception as e:
            return (
                None,
                f"Exception occured while trying to fetch file content from AWS bucket {bucket_name}. {str(e)}",
            )
    ...
    def download_file_from_s3(self, s3_file_uri):
        ...
        try:
            ...
            if (
                ...
                msg = error.response["Error"]["Message"]
                error_message = (
                    f"{msg}. BucketName: {bucket_name}. FileName: {file_name}"
                )
                logging.exception(f"ClientError exception occurred: {error_message}")
            else:
                logging.exception(f"ClientError exception occurred: {error}")
    ...
    def list_efs_file_systems(self):
        ...
        for region in self.region:
            try:
                ...
                if error:
                    errors_list.append(
                        f"Error while creating session in {region}. {error}"
                    )
                    continue
                ...
                if "FileSystems" in response:
                    ...
                    errors_list.append(
                        f"Failed to fetch EFS list in {region}. Please contact support for further details"
                    )
    ...
    def look_up_events(self, from_date, to_date, look_up_attributes=None):
        ...
        for region in self.region:
            try:
                ...
                for page in paginator.paginate(
                    LookupAttributes=look_up_attributes,
                    StartTime=from_date,
                    EndTime=to_date,
                ):
                    if "Events" in page:
    ...
    def _get_endpoint_error_message(self, error, region):
        if (
            hasattr(error, "kwargs")
            and isinstance(error.kwargs, dict)
            and "endpoint_url" in error.kwargs
        ):
            endpoint_url = error.kwargs["endpoint_url"]
    ...
    def list_users(self):
        ...
        if error:
            return [], [
                {
                    "Error": f"Error while creating a session to fetch AWS user list. {str(error)}"
                }
            ]
        try:
            ...
            if not user_list:
                return user_list, [
                    {
                        "Error": "Failed to fetch user(s) for the provided AWS credentials."
                    }
                ]
            return user_list, []
            ...
            error_message = (
                "ServiceFailureException occurred while fetching AWS user list."
            )
            if (
                hasattr(error, "response")
                and isinstance(error.response, dict)
            ...
            error_message = (
                "Could not connect to the AWS fetch users(list_users) endpoint URL"
            )
            if (
                hasattr(error, "kwargs")
                and isinstance(error.kwargs, dict)
    ...
    def list_attached_user_policies(self, user_name, iam_client=None):
        ...
        if not user_name:
            return [], [
                {"Error": "UserName is mandatory to fetch user attached policies"}
            ]
        if iam_client is None:
            ...
            if error:
                return [], [
                    {
                        "Error": f"Failed to create session while fetching user attached policies. {error}"
                    }
                ]
            iam_client = session.client("iam")
        try:
            ...
            for page in paginator.paginate(UserName=user_name):
                if "AttachedPolicies" in page and isinstance(
                    page["AttachedPolicies"], list
                ):
                    aws_managed_policies.extend(page["AttachedPolicies"])
            if not aws_managed_policies:
                return [], [
                    {"Error": f"No attached policies found for user - {user_name}"}
                ]
    ...
    def list_user_policies(self, user_name, iam_client):
        ...
        if iam_client is None:
            ...
            if error:
                return [], [
                    {
                        "Error": f"Failed to create session while fetching user policies. {error}"
                    }
                ]
            iam_client = session.client("iam")
        try:
            ...
            error_message = (
                f"Client error occurred while fetching policies for user - {user_name}."
            )
            if (
                hasattr(error, "response")
                and isinstance(error.response, dict)
    ...
    def list_user_policy_names(self, user_name):
        ...
        try:
            ...
            aws_managed_policies, error_list = self.list_aws_managed_user_policy_names(
                user_name
            )
            if error_list:
                return [], error_list
    ...
    def list_aws_managed_user_policy_names(self, user_name):
        ...
        try:
            policies = []
            aws_managed_policies, error_list = self.list_attached_user_policies(
                user_name
            )
            if error_list:
                return [], error_list
    ...
    def list_policies(self, scope=None):
        ...
        if error:
            return [], [
                {
                    "Error": f"Failed to create a session while fetching AWS policy list: {str(error)}"
                }
            ]
        iam_client = session.client("iam")
        try:
            ...
            if not policies:
                return [], [
                    {"Error": f"No policies found for provided AWS credentials."}
                ]
            if policies:
                for policy in policies:
                    ...
                    policy_groups, policy_roles, policy_users, pol_ent_err_list = (
                        self.list_entities_for_policy(policy_arn, iam_client)
                    )
                    if pol_ent_err_list:
            ...
            # https://botocore.amazonaws.com/v1/documentation/api/latest/reference/services/iam/client/exceptions/ServiceFailureException.htmlHandle ServiceFailureException
            error_message = (
                "ServiceFailureException occurred while fetching AWS policies."
            )
            if (
                hasattr(error, "response")
                and isinstance(error.response, dict)
            ...
            error_message = (
                "EndpointConnectionError occured while fetching AWS policies."
            )
            if (
                hasattr(error, "kwargs")
                and isinstance(error.kwargs, dict)
    ...
    def list_entities_for_policy(self, policy_arn, iam_client=None):
        ...
        if not policy_arn:
            return (
                ...
                [],
                [],
                [{"Error": "PolicyARN is mandatory to fetch policy entities"}],
            )
        if iam_client is None:
            session, error = self.create_aws_session()
            if error:
                return (
                    ...
                    [
                        {
                            "Error": f"Failed to create a session while fetching policy entities for policy - {policy_arn}. {str(error)}"
                        }
                    ],
                )
            iam_client = session.client("iam")
    ...
    def get_policy_version(self, policy_arn, version_id, iam_client=None):
        ...
        if not version_id:
            return [], [
                {"Error": "Policy Version ID is mandatory to fetch policy version"}
            ]
        if iam_client is None:
            session, error = self.create_aws_session()
            if error:
                return [], [
                    {
                        "Error": f"Failed to create a session while fetching policy version for policy - {policy_arn}"
                    }
                ]
            iam_client = session.client("iam")
        try:
            ...
            if (
                ...
                and "PolicyVersion" in response
                and response["PolicyVersion"]
                and isinstance(response["PolicyVersion"], dict)
                and "Document" in response["PolicyVersion"]
                and response["PolicyVersion"]["Document"]
                and isinstance(response["PolicyVersion"]["Document"], dict)
                and "Statement" in response["PolicyVersion"]["Document"]
                and isinstance(response["PolicyVersion"]["Document"]["Statement"], list)
            ):
    ...
    def list_user_policy_names(self, user_name, policy_list):
        ...
        try:
            if not user_name or not policy_list:
                return [], [
                    {"Error": "Mandatory parameter missing: UserName or PolicyList"}
                ]
            df = pd.DataFrame(policy_list)
            if "PolicyUsers" not in df.columns:
                return [], [
                    {"Error": "Invalid policy list. 'PolicyUsers' column is missing"}
                ]
            ...
            return [], [
                {
                    "Error": f"Failed to fetch user policy details for user {user_name}. {str(e)}"
                }
            ]
    ...
    def list_groups_for_user(self, user_name, iam_client=None):
        ...
        if iam_client is None:
            ...
            if error:
                return [], [
                    {
                        "Error": f"Failed to create a session while fetching groups for user - {user_name}"
                    }
                ]
            iam_client = session.client("iam")
        try:
            ...
            error_message = (
                f"Client Exception occured while etching groups for user - {user_name}."
            )
            if (
                hasattr(error, "response")
                and isinstance(error.response, dict)
    ...
    def list_group_policies(self, groups, policy_list):
        try:
            if not groups or not policy_list:
                return [], [
                    {"Error": "Mandatory parameter missing: GroupName(s) or PolicyList"}
                ]
            if not isinstance(groups, list):
                return [], [
                    {"Error": "Invalid type: 'group_names'. Supported type list"}
                ]
            policies = []
            df = pd.DataFrame(policy_list)
            if "PolicyGroups" not in df.columns:
                return [], [
                    {"Error": "Invalid policy list. 'PolicyGroups' column is missing"}
                ]
            # Create a boolean mask for rows where any of the provided groups is in 'PolicyGroups'
            df["GroupInPolicy"] = df["PolicyGroups"].apply(
                lambda groups_list: any(group in groups for group in groups_list)
            )
            ...
            return [], [
                {
                    "Error": f"Failed to fetch group policy details for group(s) - {', '.join(groups)}. {str(e)}"
                }
            ]
    ...
    def list_user_roles(self, user_name, iam_client=None):
        ...
        if iam_client is None:
            ...
            if error:
                return [], [
                    {
                        "Error": f"Failed to create a session while fetching roles for user - {user_name}"
                    }
                ]
            iam_client = session.client("iam")
        try:
            ...
            return [], [
                {"Error": f"Failed to fetch roles for user - {user_name}. {str(error)}"}
            ]
        except botocore.exceptions.EndpointConnectionError as error:
    ...
    def list_role_policies(self, roles, policy_list):
        try:
            if not roles or not policy_list:
                return [], [
                    {"Error": "Mandatory parameter missing: Role(s) or PolicyList"}
                ]
            ...
            if "PolicyRoles" not in df.columns:
                return [], [
                    {"Error": "Invalid policy list. 'PolicyRoles' column is missing"}
                ]
            # Create a boolean mask for rows where any of the provided roles is in 'PolicyRoles'
            df["RoleInPolicy"] = df["PolicyRoles"].apply(
                lambda role_list: any(role in roles for role in role_list)
            )
            # Filter the DataFrame where the boolean mask is True
            ...
            return [], [
                {
                    "Error": f"Failed to fetch role policy details for role(s) - {', '.join(roles)}. {str(e)}"
                }
            ]
    ...
    def list_action_for_polcies(self, policy_name_list, aws_policies=None):
        ...
        try:
            ...
            if aws_policies is None:
                ...
                if error:
                    return [], [
                        {"Error": f"Error while fetching policy actions. {error}"}
                    ]
            aws_policies_df = pd.DataFrame(aws_policies)
            filtered_policies = aws_policies_df[
                aws_policies_df["PolicyName"].isin(policy_name_list)
            ]
            action_list = []
            ...
            for aws_policy in filtered_policies_list:
                ...
                actions, error = self.get_policy_version(
                    policy_arn=policy_arn, version_id=policy_version_id
                )
                if error:
                    return [], [
                        {
                            "Error": f"Error while fetching policy actions for policy {aws_policy['PolicyName']}. {error}"
                        }
                    ]
                if actions:
            ...
            return action_list, [
                {
                    "Error": f"Failed to fetch action list for input policy list. {str(e)}"
                }
            ]
    ...
    def list_analyzers(self, region, analyzer_type=None):
        ...
        try:
            ...
            if error:
                return [], (
                    [{"Error": f"Error while creating session in {region}. {error}"}]
                )
            ...
            try:
                ...
                if isinstance(error, dict) and cowdictutils.is_valid_key(
                    error, "message"
                ):
            ...
            if (
                ...
                endpoint_url = error.kwargs["endpoint_url"]
                error_message = (
                    f"Could not connect to the endpoint URL: '{endpoint_url}."
                )
            error_list.append({"Error": error_message})
        except botocore.exceptions.ClientError as error:
            error_message = (
                f"Client Exception occured while fetching analyzer details in {region}"
            )
            if (
                hasattr(error, "response")
                and isinstance(error.response, dict)
    ...
    def list_findings_v2(self, analyzer_type):
                                ...
                                if finding_id:
                                    try:
                                        ...
                                        if isinstance(
                                            ...
                                            error_list.append(
                                                {
                                                    "Error": f"{msg}. {error.get('message')}"
                                                }
                                            )
                                        else:
                                            error_list.append(
                                                {"Error": f"{msg}. {str(error)}"}
                                            )
                                    except (
        ...
        if invalid_regions:
            error_list.append(
                {
                    "Error": f"Failed to fetch analyzer finding details for the following region(s): {', '.join(invalid_regions)}"
                }
            )

        return data, error_list

    def get_unused_permissions_for_user_using_access_analyzer(
        self, finding_info, user_name
    ):
        ...
        try:
            ...
            filtered_df = df[
                (df["findingType"] == "UnusedPermission")
                & (df["resource_name"] == user_name)
                & (df["status"] == "ACTIVE")
            ]
                            ...
                            if action_list is not None:
                                for action in action_list:
                                    permission_details.append(
                                        f"{service_name_space}:{action.get('action')}"
                                    )
                        else:
            ...
            return [], [
                {
                    "Error": f"Error occured while fetching access analyzer findings for user - {user_name}"
                }
            ]
    ...
    def modify_user_permissions(self, permission_df, user_permissions):
        ...
        if missing_columns:
            return [], [
                {
                    "Error": f"Invalid master permission data. Missing column(s): {', '.join(missing_columns)}"
                }
            ]
        ...
        try:
            for permission in user_permissions:
                ...
                if service not in all_permissions_included:
                    filtered_df = permission_df[
                        permission_df["Service"] == service.upper()
                    ]
                    ...
                    if filtered_df.empty:
                        ...
                        if access_level:
                            access_filtered_df = filtered_df[
                                filtered_df["AccessLevel"] == access_level
                            ]
                            if access_filtered_df.empty:
                                ...
                                filtered_permissions = [
                                    perm
                                    for perm in permission_list
                                    if perm.lower().startswith(access_level)
                                ]
            ...
            return [], [
                {
                    "Error": f"Failed to modify user permission from master permission list. {str(error)}"
                }
            ]
    ...
    def sign_aws_request(
        self, aws_session_token: str, region: Optional[str] = None
        ...
        try:
            ...
            credentials = Credentials(
                aws_role.access_key, aws_role.secret_key, aws_session_token
            )
    ...
    def exchange_for_gcp_token(
        self, subject_token_encoded: str, gcp_creds: Dict[str, Any]
    ...
    def generate_service_account_token(
        self, gcp_access_token: str, gcp_creds: Dict[str, Any]
    ) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        try:
    ...
    def create_bigquery_job(
        ...
        try:
            ...
            response = requests.post(
                bigquery_url, headers=bigquery_headers, json=query_payload
            )
            response.raise_for_status()
    ...
    def check_query_job_status(
        ...
        try:
            ...
            final_job_status_url = self.replace_placeholders_in_url(
                job_status_url, job_query_response
            )
            ...
            while retries < max_retries:
                ...
                job_state = self.extract_value_from_json(
                    job_status_response, condition_field
                )
    ...
    def fetch_query_response_using_job_id(
        self, job_query_response: Dict[str, Any], service_account_token: str
    ) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        try:
            ...
            final_query_results_url = self.replace_placeholders_in_url(
                query_results_url_template, job_query_response
            )
    ...
    def extract_value_from_json(
        self, response_json: Dict[str, Any], placeholder: str
    ) -> str:
        try:
    ...
    def replace_placeholders_in_url(
        self, url_template: str, response_json: Dict[str, Any]
    ) -> str:
        """Replaces placeholders in URL with values from the response JSON."""
        return re.sub(
            r"<<(.+?)>>",
            lambda match: self.extract_value_from_json(response_json, match.group(1)),
            url_template,
        )

    def get_aws_security_hub_resource_type(self, resource_type: str):
        aws_security_hub_resources = {
            # Original entries
            "AwsCloudTrailTrail": "AWS::CloudTrail::Trail",
            "AwsConfigConfigurationRecorder": "AWS::Config::ConfigurationRecorder",
            "AwsConfigDeliveryChannel": "AWS::Config::DeliveryChannel",
            ...
            # Additional important Security Hub resources
            ...
            "AwsDocDbDbCluster": "AWS::DocDB::DBCluster",
            ...
            "AwsGlueCrawler": "AWS::Glue::Crawler",
            "AwsIamManagedPolicy": "AWS::IAM::ManagedPolicy",
            "AwsKinesisStream": "AWS::Kinesis::Stream",
            "AwsLogsLogGroup": "AWS::Logs::LogGroup",
            "AwsMacieFindingsFilter": "AWS::Macie::FindingsFilter",
            ...
            "AwsSsmAssociation": "AWS::SSM::Association",
            "AwsStepFunctionsStateMachine": "AWS::StepFunctions::StateMachine",
            "AwsSystemsManagerManagedInstance": "AWS::SSM::ManagedInstance",
            "AwsTransferServer": "AWS::Transfer::Server",
            "AwsVpcFlowLog": "AWS::EC2::FlowLog",
        }

        return aws_security_hub_resources.get(resource_type, "N/A")
