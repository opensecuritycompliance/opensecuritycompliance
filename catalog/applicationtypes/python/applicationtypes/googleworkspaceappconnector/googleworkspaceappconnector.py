...
class GoogleWorkSpaceAppConnector:
    ...
    def from_dict(obj) -> "GoogleWorkSpaceAppConnector":
        ...
        return GoogleWorkSpaceAppConnector(app_url, app_port, user_defined_credentials)
    ...
    def download_file_from_url(self, file_url):
        try:
            ...
            if mime_type != "application/vnd.google-apps.folder":
                ...
                latest_file_id, latest_file_name, latest_mime_type = (
                    self.find_latest_file_recursively(service, target_id)
                )
    ...
    def download_file_from_path(self, file_path: str, target_drive_id: str = "root"):
        try:
            scope = "https://www.googleapis.com/auth/drive.readonly"
            token_source, err = self.create_config(scope)
            if err:
            ...
            parent_folder_id = target_drive_id
            if folder_path:
                parent_folder_id, err = self.get_folder_id(folder_path, target_drive_id)
                if err:
                    return None, err
                if not parent_folder_id:
                    return None, f"Folder path '{folder_path}' not found."
            ...
            if not files:
                return None, f"File '{file_name}' not found in the specified path."
    ...
    def find_latest_file_recursively(self, service, folder_id):
        ...
        for file_item in files:
            ...
            if latest_time is None or file_latest_time > latest_time:
                ...
                print(
                    f"DEBUG: New candidate: {candidate_name} (Time: {file_latest_time})"
                )
        ...
        for folder_item in folders:
            ...
            if sub_result[0]:  # If we found a file in subfolder
                ...
                sub_created_time = self.parse_drive_timestamp(
                    sub_file_metadata.get("createdTime")
                )
                sub_modified_time = self.parse_drive_timestamp(
                    sub_file_metadata.get("modifiedTime")
                )
    ...
    def get_last_uploaded_in_folder(self, service, folder_id):
        ...
        try:
            ...
            if current_files:
                ...
                current_latest_time = self.parse_drive_timestamp(
                    current_latest_file.get("createdTime")
                )
            ...
            folder_results = (
                service.files()
                .list(q=query_folders, fields="files(id, name)", pageSize=10)
                .execute()
            )
            ...
            for subfolder in subfolders:
                sub_time, sub_id, sub_name, sub_mime = self.get_last_uploaded_in_folder(
                    service, subfolder["id"]
                )
                if sub_time and (
                    subfolder_latest_time is None or sub_time > subfolder_latest_time
                ):
            ...
            if current_latest_time and subfolder_latest_time:
                if current_latest_time >= subfolder_latest_time:
                    return (
                        current_latest_time,
                        current_latest_file["id"],
                        current_latest_file["name"],
                        current_latest_file["mimeType"],
                    )
                else:
                    return (
                        subfolder_latest_time,
                        subfolder_latest_file[0],
                        subfolder_latest_file[1],
                        subfolder_latest_file[2],
                    )
            ...
            logging.error(
                f"Error getting last uploaded for folder {folder_id}: {str(e)}"
            )
            return None, None, None, None
    ...
    def upload_file_to_drive(
        self, file_content, file_name, folder_name, drive_id="root"
    ):
        try:
            ...
            folder_id, err = self.get_folder_id(folder_name, drive_id)
            if err:
                return None, err
    ...
    def get_folder_id(self, folder_path, drive_id="root"):
        try:
            scope = "https://www.googleapis.com/auth/drive"
            token_source, err = self.create_config(scope)
            if err:
            ...
            parent_id = drive_id
            current_folder_id = drive_id

            for name in folder_names:
                # add "trashed = false" so it ignores deleted folders in the recycle bin!
                if parent_id == "root":
                    query = f"name = '{name}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
                else:
                    # For subfolders, keep the parent restriction so it traverses correctly
    ...
    def list_firewall_rules(self, project_id: str) -> Tuple[list, str]:
        try:
            ...
            if not firewall_rules:
                return (
                    [],
                    f"No service accounts found in the provided project: {project_id}.",
                )
    ...
    def list_instances(self, project_id):
        try:
            ...
            return (
                None,
                f"HTTP error occurred while fetching the instance list for project ID {project_id}: {e}.",
            )
        except AttributeError as e:
            return (
                None,
                f"Attribute error occurred while fetching the instance list for project ID {project_id}: {e}.",
            )
    ...
    def list_databases(self, project_id, instance_id):
        ...
        try:
            ...
            return (
                None,
                f"HTTP error occurred while fetching the database list for project ID {project_id}: {e}.",
            )
        except AttributeError as e:
            return (
                None,
                f"Attribute error occurred while fetching the database list for project ID {project_id}: {e}.",
            )
    ...
    def get_logging_entities(self, project_id, filter, from_date, to_date):
        try:
            ...
            request = service.entries().list(body=request_body)
            response = request.execute()
    ...
    def is_valid_user(self, project_id, instance_id, user_name):
        ...
        try:
            ...
            request = service.users().get(
                project=project_id, instance=instance_id, name=user_name
            )
            response = request.execute()
            if response:
                return True
    ...
    def get_activities(
        ...
        try:
            ...
            try:
                self.service = build("admin", "reports_v1", credentials=token_source)
            ...
            batch_size = 1000
            batches = [
                emailID_list[i : i + batch_size]
                for i in range(0, len(emailID_list), batch_size)
            ]
            for batch_emails in batches:
                self.batch = BatchHttpRequest(
                    callback=self.handle_batch_response,
                    batch_uri="https://admin.googleapis.com/batch",
                )
