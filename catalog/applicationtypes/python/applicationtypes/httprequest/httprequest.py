...
class OAuth:
    ...
    def __init__(self, validation_curl: str, client_id: str,
...
class JWTBearer:
    ...
    def __init__(self, algorithm: str, private_key: str, payload: str,
                 validation_curl: str) -> None:
...
class AWSSignature:
    ...
    def __init__(self, validation_curl: str, access_key: str,
                 secret_key: str) -> None:
        self.validation_curl = validation_curl
...
class BasicAuthentication:
    ...
    def __init__(self, validation_curl: str, user_name: str,
                 password: str) -> None:
        self.validation_curl = validation_curl
...
class UserDefinedCredentials:
    ...
    def __init__(self, o_auth: OAuth, custom_type: CustomType, api_key: APIKey,
                 no_auth: NoAuth, jwt_bearer: JWTBearer,
                 aws_signature: AWSSignature, bearer_token: BearerToken,
                 basic_authentication: BasicAuthentication) -> None:
        self.o_auth = o_auth
    ...
    def from_dict(obj) -> 'UserDefinedCredentials':
        o_auth, custom_type, api_key, no_auth, jwt_bearer, aws_signature, bearer_token, basic_authentication = None, None, None, None, None, None, None, None
        if isinstance(obj, dict):
            ...
            aws_signature = AWSSignature.from_dict(
                obj.get("AWSSignature", None))
        ...
        return UserDefinedCredentials(o_auth, custom_type, api_key, no_auth,
                                      jwt_bearer, aws_signature, bearer_token,
...
class HttpRequest:
        ...
        if isinstance(obj, dict):
            ...
            if bool(user_defined_credentials_dict):
                ...
                if 'NoAuth' in user_defined_credentials_dict:
                    user_defined_credentials_dict = UserDefinedCredentials(o_auth = None, custom_type = None, api_key = None, no_auth = NoAuth(),
                                      jwt_bearer = None, aws_signature=None, bearer_token = None,
    ...
    def validate(self) -> bool and dict:
        ...
        if all((creds_jwt_bearer.algorithm, creds_jwt_bearer.payload , creds_jwt_bearer.private_key)) :
        ...
        if creds_api_key.api_key :
            is_valid,error = self.validate_credentials_type("APIKey" , creds_api_key.validation_curl)
            if error :
                return False , error
        ...
        if  creds_bearer_token.token :
            is_valid,error = self.validate_credentials_type("BearerToken" , creds_bearer_token.validation_curl)
            if error :
                return False , error
        ...
        if  all((creds_basic_auth.user_name ,  creds_basic_auth.password)) :
            is_valid,error = self.validate_credentials_type("BasicAuthentication" , creds_basic_auth.validation_curl)
            if error :
                return False , error
        ...
        if  all((creds_o_auth.client_id ,  creds_o_auth.client_secret)) :
            is_valid,error = self.validate_credentials_type("OAuth" , creds_o_auth.validation_curl)
            if error :
                return False , error
    ...
    def validate_basic_auth(self):
        ...
        validation_curl = self.user_defined_credentials.basic_authentication.validation_curl
        if validation_curl.strip()=='':
             return False , {'Error': 'ValidationCURL cannot be empty.'} 
        parsed_curl, error = self.replace_placeholder(validation_curl , 'BasicAuthentication', self.user_defined_credentials.basic_authentication.to_dict())
        if error :
            return False , {'Error': 'Error while processing place holders.'} 
        ...
        if '<<BasicAuthentication>>' in parsed_curl :
            ...
            parsed_curl = parsed_curl.replace('<<BasicAuthentication>>' , headers['Authorization']  )
    ...
    def validate_api_key(self):
        ...
        parsed_curl, error = self.replace_placeholder(validation_curl , 'APIKey', self.user_defined_credentials.api_key.to_dict())
        if error :
            return False , {'Error': 'Error while processing place holders.'} 
    ...
    def validate_bearer_token(self):
        ...
        if validation_curl.strip()=='':
             return False , {'Error': 'ValidationCURL cannot be empty.'} 
        parsed_curl, error = self.replace_placeholder(validation_curl , 'BearerToken', self.user_defined_credentials.bearer_token.to_dict())
        if error :
            return False , {'Error': 'Error while processing place holders.'} 
        ...
        if '<<BearerToken>>' in parsed_curl :
            ...
            parsed_curl = parsed_curl.replace('<<BearerToken>>' , headers['Authorization']  )
    ...
    def validate_o_auth(self):
        ...
        if validation_curl.strip()=='':
             return False , {'Error': 'ValidationCURL cannot be empty.'} 
        parsed_curl, error = self.replace_placeholder(validation_curl , 'OAuth', self.user_defined_credentials.o_auth.to_dict())
        if error :
            return False , {'Error': 'Error while processing place holders.'} 
    ...
    def validate_credentials_type(self,credential_type ,validation_curl ) :
        ...
        if credential_type == 'BasicAuthentication':
                    ...
                    if i+1 <= len(tokens) :
                        if 'Basic' in tokens[i + 1]: 
                            if  result["Authorization"] in tokens[i + 1]:
                                return True , ""
                            return False , "Credentials in ValidationCURL and Credential miss match"
                ...
                if token.lower() == '-h' or token.lower() == '--header':
                    if 'Bearer' in tokens[i + 1]:
                        if  bearer_token in tokens[i + 1]:
                            return True , ""
                        return False , "Token in ValidationCURL and Credential miss match"
            ...
            if not body_data:
                return False, "ValidationCURL has no request body, OAuth expecte the 'url-encoded' request body."
            ...
            if re.search(client_secret_pattern, body_data) :
                client_secret_full_pattern = fr"{client_secret_pattern}\s*=\s*{re.escape(client_secret)}"
            ...
            if re.search(client_id_pattern, body_data):
                client_id_full_pattern = fr"{client_id_pattern}\s*=\s*{re.escape(client_id)}"
            ...
            if miss_match_cred :
                return False , f"{' and '.join(miss_match_cred)} miss match in ValidationCURL and Credential." 
    ...
    def validate_curl(self , parsed_curl ) :
        ...
        if is_resp_valid:
            ...
            error_message = error_messages.get(
                status_code, f"CURL command failed with HTTP status code {status_code}. {response_body}" )
    ...
    def make_api_call(self ,request_data):
        ...
        response,error = self.send_request_with_retries(
            method, url, headers, params, body, timeout, retries, verify , retry_on_status , allow_redirects , files)
    ...
    def send_request_with_retries(self ,
        method, url, headers, params, body, timeout, retries,verify, retry_on_status, allow_redirects, files):
        ...
        if retries and isinstance(retries, int):
            ...
            condition_field = retries.get("RetryOnCondition",{}).get("ConditionField", "")
            
        while (attempt < max_retries) or (max_retries == 0 and attempt == 0):
            try:
                
                response = self.send_request(method, url, headers, params, body, timeout, allow_redirects, files, verify)
                if isinstance(retries, int):
                    if (response.status_code in retry_on_status) or (response.status_code in [502 , 504, 429]): 
                ...
                elif isinstance(retries, dict) and ("<<" in condition_field and ">>" in condition_field):
                    
                    condition_value = retries.get("RetryOnCondition",{}).get("ConditionValue", "")
                    delay = retries.get("RetryOnCondition",{}).get("TimeInterval", 2)
                    modified_condition_field = condition_field.replace("responsebody", ".body").replace("response", "").replace("<<", "").replace(">>", "")
                    ...
                    if 'application/json' in content_type or 'application/ld+json' in content_type:
                        ...
                        return None , f"Expected 'application/json' or 'application/ld+json' for .body access, but received: '{content_type}'."
                    
                    try:
                        if not modified_condition_field:
                            modified_condition_field = "."
                        condition_field_response = jq.compile(modified_condition_field).input(response_dict).first()
                    except ValueError as e:
                        return None , f"Error while parsing the condition field: {str(e)}"

                    if str(condition_field_response) in condition_value.split("|") or (response.status_code in [502, 504, 429]):   
                        if not isinstance(delay, int):
                            header_key = delay.replace("<<response.headers.", "").replace(">>", "")
                            ...
                            if retry_after:
                                try:
                                    ...
                                    try:
                                        delay = max(0, int(float(retry_after)) - int(time.time()))
                                    except ValueError:
                                        delay = 2 
                            else:
                ...
                print(f"Request failed: {str(e)}. Retrying... ({attempt + 1}/{retries})")
    ...
    def send_request(self ,method, base_url, headers, params, body, timeout, allow_redirects , files , verify):
    ...
    def generate_aws_iam_signature(self ,auth_header , service , region , api_url ,
                                   request_body, request_parameters , method) :
        
        aws_services = ["ec2", "s3", "lambda", "rds", "dynamodb", "monitoring", "sns", "sqs", "route53", 
                        "cloudfront", "elasticbeanstalk", "iam", "elasticloadbalancing", "kinesis", 
                        "redshift","execute-api", "config", "iot", "glue", "codecommit", "codebuild",  
                        "cloudformation", "cloudtrail", "auditmanager", "ce", "states", "secretsmanager", 
                        "ssm", "elasticache", "guardduty", "waf", "xray", "transfer", "eks", "ecr", "appsync", 
                        "organizations", "chime", "sagemaker", "amplify", "databrew", "ecs", "batch" ,"codepipeline",
                        "securityhub"
                        ]
    ...
    def generate_jwt_token(self, algorithm, private_key, payload_str):

        try:
            updated_payload, error = self.replace_function_placeholders(str(payload_str))
            if error:
                return "", error
            payload = {}
            try:
    ...
    def generate_jwt_bearer(self):
        ...
        if is_resp_valid:
            ...
            return None , f"CURL command failed with HTTP status code {status_code}. {response_body}"
    ...
    def get_credential_json_data(self) :
        ...
        if self.user_defined_credentials.custom_type.credential_json :
            ...
            try:
                ...
                return None , 'Error while reading "CredentialJson" data, Invalid JSON data.'
    ...
    def generate_signature(self, method, service, host, 
                           region, canonical_uri, canonical_headers ,request_parameters,
                           request_body, access_key, secret_key ):
        
        no_region_services = ['s3', 'iam', 'route53', 'cloudfront', 'cognito-idp', 'dynamodb']
        if service in no_region_services:
            region = "us-east-1"
    ...
    def sort_dict_lexicographically(self,input_dict):
        return dict(sorted(input_dict.items(), key=lambda item: item[0].lower()))
    ...
    def replace_placeholder(self, target_str, placeholder_prefix, value_dict):
        ...
        for placeholder_key in matches:
            ...
            if parsed_value  :
                target_str = target_str.replace(f"<<{placeholder_prefix}{placeholder_key}>>" , str(parsed_value).strip())
            else:
    ...
    def evaluate_cel_expression(self, expression: str, context: dict = {}) -> Tuple[celpy.celtypes.Value, str]:
        try:
            cel_env = celpy.Environment()
            cel_ast = cel_env.compile(expression.replace('<<', '').replace('>>', ''))
            result = cel_env.program(cel_ast)\
                .evaluate(
                    celpy.json_to_cel(context)
                )
    ...
    def replace_function_placeholders(self, string: str):
        functions = {
        ...
        for match in placeholder_matches:
            try:
                ...
                if error:
                    return '', f"An error occurred while replacing '<<{match}>>' :: {error}"
            ...
            try:
                ...
                for key in keys:
                    ...
                    if isinstance(current_data, dict):
                        if '[' in key and ']' in key:
                            ...
                            if index == 'x':
                                ...
                                for sub_item in current_data[key_name]:
                                    ...
                                    result, err = self.extract_value(f"<<{sub_query}>>", sub_item)
                                    if err:
                                        return None, err
                        ...
                        return None, f"Expected dict but got {type(current_data).__name__}"
    ...
    def validate_credetials_type(self, task_credential_type):
        ...
        if custom_type.validation_curl and "CustomType" == task_credential_type:
            return True
        elif all((creds_aws_signature.access_key, creds_aws_signature.secret_key)) and "AWSSignature" == task_credential_type :
            return True
        elif  creds_bearer_token.token and "BearerToken" == task_credential_type :
            return True
        elif  all((creds_basic_auth.user_name ,  creds_basic_auth.password)) and "BasicAuthentication" == task_credential_type:
            return True
        elif  all((creds_o_auth.client_id ,  creds_o_auth.client_secret)) and "OAuth" == task_credential_type :
            return True
        elif  creds_api_key.api_key and "APIKey" == task_credential_type :
            return True
        elif "NoAuth" == task_credential_type :
            return True
        elif creds_jwt_bearer.algorithm and creds_jwt_bearer.payload and creds_jwt_bearer.private_key and "JWTBearer" == task_credential_type :
            return True
        else:
            return False
