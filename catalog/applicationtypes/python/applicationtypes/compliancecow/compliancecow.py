...
class ComplianceCow:
    ...
    CREATE_CONTROL_NOTE = '/v5/partner/assessment-runs/{assessment_run_id}/controls/{control_id}/notes'
    ...
    GET_PLANS = '/v1/plans'
    GET_ASSESSMENT_DETAILS = '/v5/partner/assessment-runs?page={page}&page_size={page_size}&assessment_id={assessment_id}'
    GET_ASSESSMENT_RUN_DETAILS = '/v5/partner/assessment-runs/{assessment_run_id}'
    CREATE_ASSESSMENT = '/v5/partner/assessments'
    FETCH_ASSESSMENT = '/v5/partner/assessments?name={assessment_name}&include_all_controls=true'
    CREATE_ASSESSMENT_CONTROL = '/v5/partner/assessments/{assessment_id}/controls'
    UPDATE_ASSESSMENT_CONTROL = '/v5/partner/assessments/{assessment_id}/controls/{control_id}'
    ...
    CREATE_CITATION = '/v5/partner/assessments/{assessment_id}/controls/{control_id}/citations'
    ...
    DELETE_ASSESSMENT_CONTROL = '/v5/partner/assessments/{assessment_id}/controls/{control_id}'
    UPDATE_EVIDENCE = '/v5/partner/assessment-runs/{assessment_run_id}/controls/{control_id}/evidence'
    CREATE_EVIDENCE_TEMPLATE = '/v5/partner/assessments/{assessment_id}/controls/{control_id}/evidence'
    ...
    LINK_EVIDENCE_RECORDS = '/v5/partner/assessment-runs/{assessment_run_id}/link-records'
    ...
    DELETE_FORM = '/v1/forms/{form_id}' # Change to Partner API after this is added to it
    ...
    GET_USER_MEDIUM_CONFIGS = '/v1/user-medium-configurations?medium_user_id={medium_user_id}'
    ...
    def fetch_auth_token(self):
        ...
        if error:
            print(
                f"Unable to retrieve ComplianceCow auth token : {error}")
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            ...
            return None, f"Validation failed. Exception occurred while validating app. {response.status_code} :: {response.text}"
    ...
    def fetch_control_user_inputs(self, assessment_run_id, control_id):
        ...
        if error:
            print(
                f"Unable to retrieve ComplianceCow control user input : {error}")
            return None, 'Unable to retrieve ComplianceCow control user input. Please contact admin/support to fix this issue.'
    ...
    def update_control_meta_data(self, assessment_run_id, control_id, payload_data):
        ...
        if error:
            print(
                f"Unable to update the ComplianceCow control metadata : {error}")
    ...
    def update_assessment_run_meta_data(self, assessment_run_id, payload_data):
        ...
        if error:
            return error
        api_endpoint_url = self.build_api_url(self.UPDATE_ASSESSMENT_RUN_META_DATA).format(
    ...
    def upload_control_attachment(self, assessment_run_id, control_id, payload_data, files):
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return error
        ...
        response, error = self.make_api_request(
            api_endpoint_url, headers=headers, method='POST', data=payload_data, files=files)
    ...
    def delete_control_attachment(self, attachment_id, assessment_run_id, control_id):
        ...
        api_endpoint_url = self.build_api_url(self.DELETE_CONTROL_ATTACHMENT).format(
            assessment_run_id=assessment_run_id, control_id=control_id, attachment_id=attachment_id)
        headers = {
            'Authorization': auth_token
        }
        response, error = self.make_api_request(
    ...
    def create_control_note(self, assessment_run_id, control_id, payload_data):
        ...
        if error:
            print(
                f"Unable to create the note for the ComplianceCow control : {error}")
    ...
    def update_control_note(self, assessment_run_id, control_id, note_id, payload_data):
        ...
        if error:
            print(
                f"Unable to update the note for the ComplianceCow control : {error}")
    ...
    def delete_control_note(self, note_id, assessment_run_id, control_id):
        ...
        if error:
            print(
                f"Unable to delete the note for the ComplianceCow control : {error}")
    ...
    def get_form(self, form_id: str) -> Tuple[dict, str]:
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            ...
            return None, f"Received error from response :: Status Code: {response.status_code}, Response: {response.text}"
        
    def get_form_element(self, element_id: str) -> Tuple[dict, str]:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        api_endpoint_url = self.build_api_url(self.FETCH_FORM_ELEMENT).format(element_id=element_id)
        headers = {
            'Authorization': auth_token
        }
        response, error = self.make_api_request(
    ...
    def get_form_response(self, form_id: str) -> Tuple[dict, str]:
        ...
        if error:
            return None, error
        api_endpoint_url = self.build_api_url(self.FETCH_FORM_RESPONSE).format(form_id=form_id)
        headers = {
            'Authorization': auth_token
        }
        response, error = self.make_api_request(
    ...
    def create_form_response(self, form_id: str, user_id: str, assign_id: str) -> Tuple[str, str]:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return '', error
        api_endpoint_url = self.build_api_url(self.CREATE_FORM_RESPONSE).format(form_id=form_id)
        headers = {
            'Authorization': auth_token
        }
        payload = {
    ...
    def update_form_response(self, form_id: str, response_id: str, assign_id: str, data: dict) -> Tuple[str, str]:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        api_endpoint_url = self.build_api_url(self.SAVE_FORM_RESPONSE).format(form_id=form_id, response_id=response_id)
        headers = {
            'Authorization': auth_token
        }
        ...
        if data:
            return None, f"The form does not have the following data: '{', '.join(data.keys())}'"
            ...
            if response.ok and response.status_code == http.HTTPStatus.OK:
                try:
                    ...
                    if 'pagination' not in response_dict:
                        ...
                        total_pages = int(response_dict['pagination'].get('totalPages', 0))
                        cur_page += 1
    ...
    def get_form_by_name(self, form_name: str, form_type: str = "") -> Tuple[pd.DataFrame, str]:
        ...
        if "name" not in forms_df.columns:
            return pd.DataFrame(), "Error while fetching forms :: 'name' field not found in response"
    ...
    def get_user_medium_config(self, medium_user_id: str) -> tuple[Dict[str, Any], str]:
        ...
        if error:
            return {}, error
        api_endpoint_url = self.build_api_url(self.GET_USER_MEDIUM_CONFIGS.format(medium_user_id=medium_user_id))
        headers = {
            'Authorization': auth_token
        }
    ...
    def create_evidence_template(self , 
            assessment_id:str ,control_id:str,
            evd_name:str , evd_description ,
            syn_name:str , file_content:str ) -> str :
        ...
        if response.ok and response.status_code == http.HTTPStatus.CREATED:
            ...
            return None, f'Received error from the ComplianceCow Create Assessment API response: Status Code: {response.status_code}, Response: {response.text}'

    
    
    def fetch_evidence(self, assesment_run_id: str, control_id: str, evidence_id: str) -> Tuple[pd.DataFrame, dict, str]:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, None, error
        api_endpoint_url = self.build_api_url(self.FETCH_EVIDENCE.format(
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            ...
            except (requests.JSONDecodeError, ValueError, KeyError, pyarrow.ArrowInvalid) as e:
                return None, None, f"Error while fetching evidence :: Response is in an invalid format :: {e}"
        else:
            return None, None, f"Error while fetching evidence :: Status Code: {response.status_code}, Response: {response.text}"
    ...
    def link_evidence_records(
            self ,mapper=None, assesment_id=None, assesment_run_id=None, 
            src_control_id=None,des_control_id=None, src_file_name=None, 
            des_file_name=None, src_evidence_id=None, des_evidence_id=None, 
            src_record_ids=None, des_record_ids=None ,
            src_run_control_id=None , des_run_control_id=None
        ):
    ...
    def commit_control_evidence(
        ...
        api_endpoint_url = self.build_api_url(self.COMMIT_CONTROL_EVIDENCE).format(
            assessment_run_id=assesment_run_id, control_id=control_id , evidence_id=evidence_id)
        headers = {
            'Authorization': auth_token
        }
        body = {
                "commitMessage": commit_msg,
                "commitForAll": commit_for_all
            }
        response, error = self.make_api_request(
            api_endpoint_url,
            headers=headers,
    ...
    def fetch_assessments(self, assessment_name: str = '', detailed: bool = False) -> Tuple[List[dict], str]:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        ...
        cur_page = 1
        page_size = 10 if detailed else 100
        has_next = True
        ...
        try:
            ...
            while True:
                api_endpoint_url = self.build_api_url(
                    self.GET_ASSESSMENT_DETAILS.format(
                        page=page,
                        page_size=page_size,
    ...
    def fetch_and_extract_auth_token(self):
        ...
        return None, "Unable to fetch the 'tokenType' and 'authToken' from the ComplianceCow authentication API response. Please contact admin/support to fix this issue."
    ...
    def make_api_request(self, url, headers, method='GET', json=None, data=None, files=None, params=None):
        try:
            response = None
            if method.upper() == 'GET':
                ...
                response = requests.patch(url, headers=headers, json=json, params=params)
            elif method == 'DELETE':
    ...
    def get_user_block(self, user_block_name) -> tuple[dict, str]:
        ...
        api_endpoint_url = self.build_api_url(self.GET_USER_BLOCK.format(user_block_name=user_block_name))
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        ...
        response, error = self.make_api_request(
            url=api_endpoint_url, headers=headers, method='GET', params={'isStatusToBeIncluded': 'true'})
    ...
    def get_users_from_user_mails(self, user_mails) -> tuple[dict, str]:
        ...
        api_endpoint_url = self.build_api_url(self.GET_USERS_FROM_EMAILS.format(email_in_comma_seperated=user_mails_str))
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
    ...
    def get_ids_from_users(self, users) -> tuple[dict, str]:
        ...
        try:
            ...
            return users_id, f"Exception occured while fetching IDs for user(s) - {','.join(users)}. {str(e)}"
    ...
    def get_user_mails_from_user_blocks(self, userblocks) -> tuple[dict, str]:
        ...
        try:
            ...
            return user_mails, f"Exception occured while fetching emails for userbock(s) - {','.join(userblocks)}. {str(e)}"
    ...
    def assign_control(self, control_id, users_id):
        ...
        api_endpoint_url = self.build_api_url(self.ASSIGN_CONTROL.format(control_id=control_id))
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return error
        ...
        return f'{description}. Control ID - {control_id}. User(s) - {",".join(users_id)}'
    ...
    def complete_user(self, token, control_id):
        ...
        api_endpoint_url = self.build_api_url(self.COMPLETE_USER.format(control_id=control_id))
    ...
    def get_user_mails_from_user_block_name(self, user_block_name):
        ...
        if 'items' in user_block:
            user_mails, err = self.get_user_mails_from_user_blocks([user_block['items']])
            if err:
                return None, err
            return user_mails, None
    ...
    def send_slack_notification(self, control_id, workflow_instance_id, user_mails):
        try:
            ...
            workflow_details_res, error = self.get_workflow_details(workflow_instance_id)
            if error:
                return error
            ...
            if 'metaData' in workflow_details_res:
                ...
                if 'planInstanceId' in workflow_details_res['metaData']:
                   plan_instance_id = workflow_details_res['metaData']['planInstanceId']
                if 'planInstanceControlId' in workflow_details_res['metaData']:
                   plan_instance_con_id = workflow_details_res['metaData']['planInstanceControlId']
            
            if not ass_name or not ass_run_name or not control_name or not control_num or not plan_instance_id or not plan_instance_con_id:
    ...
    def get_button_values(self, workflow_instance_id):
        ...
        text1 = {"uri":"/api/v2/workflow-instances/trigger-event","method":"POST","body":{"event":"When an assessment run control is approved","instanceId":"<workflowInstanceId>"}}
        text1['body']['syncAction'] = True
        text1['body']['instanceId'] = workflow_instance_id
        text2 = {"uri":"/api/v2/workflow-instances/trigger-event","method":"POST","body":{"event":"When an assessment run control is rejected","instanceId":"<workflowInstanceId>"}}
        text2['body']['syncAction'] = True
        text2['body']['instanceId'] = workflow_instance_id

        return self.text_to_base64(json.dumps(text1)), self.text_to_base64(json.dumps(text2))
    ...
    def get_workflow_details(self, workflow_id):
            ...
            api_endpoint_url = self.build_api_url(self.GET_WORKFLOW_DETAILS.format(workflow_id=workflow_id))
    ...
    def get_hash(self, control_id):  
        try:
            if not control_id:
                return None, "Control ID is mandatory to get the hash for control user inputs."

            api_endpoint_url = self.build_api_url(self.GET_HASH.format(control_id=control_id))
            auth_token, error = self.fetch_and_extract_auth_token()
            if error:
                return None, error
            ...
            response, error = self.make_api_request(url=api_endpoint_url, headers=headers, method='POST')
            if error:
                return None, error
            ...
            if 'ErrorMessage' in response.json():
                return '', f"Failed to get hash for control: {response.json()['ErrorMessage']}"

            return None, f'Failed to get hash for control - {control_id}'
        except Exception as e:
            return None, f"An exception occurred while fetching the hash for the user inputs of control (Control ID - '{control_id}'). {str(e)}"
    ...
    def get_user_token(self, system_objects):
        try:
            ...
            for system_object in system_objects:
                if system_object.app is not  None and system_object.app.application_name is not None and system_object.app.application_name == 'COW_API':
                    if system_object.credentials is not None:
                        creds = system_object.credentials
                        for cred in creds:
                            return cred.other_cred_info['token'], ''
                    else:
                        return '', 'Invalid COW_API credentials. Failed to fetch user token'      
                    
        except Exception as e:
           return '', f'Unable to find the COW_API credentials in the system objects. Please contact admin/support to fix this issue. {e}'
    ...
    def fetch_category_details(self, category_name: str) -> tuple[dict, str]:
        api_endpoint_url = self.build_api_url(
            self.FETCH_CATEGORY).format(category_name=category_name)
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        headers = {
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            try:
                ...
                return None, f'Error decoding the JSON response from the ComplianceCow Fetch Category Details API: {e}'
        else:
            return None, f'Received error from the ComplianceCow Fetch Category Details API response: Status Code: {response.status_code}, Response: {response.text}'
    ...
    def create_assessment_control(self, assessment_id: str, payload: dict) -> str:
        api_endpoint_url = self.build_api_url(
            self.CREATE_ASSESSMENT_CONTROL).format(assessment_id=assessment_id)
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return error
        headers = {
    ...
    def update_assessment_control(self, assessment_id: str, control_id: str, payload: list) -> str:
    ...
    def fetch_assessment_details(self, assessment_name: str) -> tuple[dict, str]:
        api_endpoint_url = self.build_api_url(
            self.FETCH_ASSESSMENT).format(assessment_name=assessment_name)
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return None, error
        headers = {
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            try:
                ...
                return None, f'Error decoding the JSON response from the ComplianceCow Fetch Assessment Details API: {e}'
        else:
            return None, f'Received an error from the ComplianceCow Fetch Assessment Details API response: Status Code: {response.status_code}, Response: {response.text}'
    ...
    def get_matched_ucf_controls(self , control_name) :
        ...
        body = {
            "query": control_name,
            "identifier":"ucf-leaf-controls-collection"
        }
    ...
    def create_control_citation(self, assessment_id, control_id, payload_data)->str:
        ...
        response, error = self.make_api_request(
            api_endpoint_url, headers=headers, method='POST', data=json.dumps(payload_data))
    ...
    def update_control_citation(self, assessment_id, control_id, citation_id, payload_data)->str:
        auth_token, error = self.fetch_and_extract_auth_token()
        if error:
            return error
        ...
        response, error = self.make_api_request(
            api_endpoint_url, headers=headers, method='PUT', data=json.dumps(payload_data))
    ...
    def trigger_workflow(self, workflow_event , input_obj )  -> tuple[list[Any], str]:
        ...
        if response.status_code == http.HTTPStatus.OK:
            try:
                ...
                return [], f"Error while decoding response data :: Response: {response.text}"
            except KeyError:
                return [], f"Response does not have the 'workflowInstanceIds' field :: Response: {response.text}"
        else:
            return [], f"Error while triggering workflow :: Status Code: {response.status_code}, Response: {response.text}"
    
    def trigger_workflow_v2(self, workflow_event:str, input_obj:Dict)->Dict:
        auth_token, error = self.fetch_and_extract_auth_token()

        if error:
            cowutils.raise_unauthorised_error(message="UNAUTHORISED_ERROR", description=error, retryable=False)
