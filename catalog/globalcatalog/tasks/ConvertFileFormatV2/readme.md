The purpose of this task is to convert files between supported formats: JSON, NDJSON, CSV, YAML, TOML, XLSX, XML, and PARQUET. It processes an input file in one of these formats and generates an output file in the specified target format.

- **InputsAndOutputsStructure**:

  - Inputs :
    - **InputFile**                : [MANDATORY] Source file containing data in formats such as JSON, NDJSON, CSV, YAML, TOML, XLSX, XML, and PARQUET
    - **OutputFileFormat**         : [MANDATORY] Target format for conversion (e.g., JSON, NDJSON, CSV, PARQUET, YAML, TOML, XLSX).
    - **PreserveType**              :[OPTIONAL] If enabled (default: false), preserves data types like array or object from CSV when converted to JSON. If disabled, they become stringified
...
- **InputsSection**:
  ...
  1. InputFile **(MANDATORY)**
     ...
     - Must be a valid MINIO file path in JSON, NDJSON, CSV, YAML, TOML, XLSX, XML, or PARQUET format.
     - **Sample InputFile:**
  ...
  2. OutputFileFormat **(MANDATORY)**
     ...
     - Allowed values: JSON, NDJSON, CSV, PARQUET, YAML, TOML, XLSX.
     - **Sample OutputFileFormat:** CSV
  3. PreserveType **(OPTIONAL)**

     - This field is optional, and the default value of PreserveType is false.
     - If PreserveType is set to true, it preserves data types like array or object from getting stringified when converting CSV to JSON.
     - Eg. CSV, a cell has a value - ['Item1', 'Item2']
       - PreserveType is FALSE: After Conversion - "['Item1', 'Item2']"
       - PreserveType is TRUE: After Conversion - ['Item1', 'Item2']
  ...
  9. ValidateFlow **(OPTIONAL)**

     - This field is optional, and the default value of ValidateFlow is false.
     - If ValidateFlow is set to true, the task will only validate the input data without executing the conversion logic. This is useful for testing and ensuring that the inputs are correct before performing the actual conversion. If the validation fails, an error will be raised, and the task will not proceed with the conversion.
- **OutputsSection**:

  1. OutputFile

     - This file contains the converted data in the target format specified in the OutputFileFormat input.
     - It will store the result of the conversion, such as the data in JSON, NDJSON, CSV, PARQUET, YAML, TOML, or XLSX format.
     - **Sample OutputFile:**
