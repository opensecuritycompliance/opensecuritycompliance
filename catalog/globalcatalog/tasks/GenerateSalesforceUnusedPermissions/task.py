...
class Task(cards.AbstractTask):
    ...
    def execute(self) -> dict:
        ...
        if log_file_url and log_file_url != "<<MINIO_FILE_PATH>>":
        ...
        permissions_map_file_url = self.task_inputs.user_inputs.get(
            "PermissionsMapFile"
        )
        ...
        try:
            ...
            return self.upload_log_file_panic({"Error": f"Failed to decode JSON: {e}"})

        permissons_file_bytes, error = self.download_file_from_minio(
            permissions_map_file_url
        )
        try:
            permissions_map = toml.loads(permissons_file_bytes.decode("utf-8"))
        except toml.TomlDecodeError as e:
            return self.upload_log_file_panic({"Error": f"Failed to decode Toml: {e}"})
        ...
        for event in eventlog.get("records", []):
            if event["Id"]:
                ...
                if not response.empty:
                    if combined_event_logs.empty and (
                        "USER_ID_DERIVED" in response.columns
                        or "USER_ID" in response.columns
                    ):
                        ...
                        combined_event_logs = pd.concat(
                            [combined_event_logs, response], ignore_index=True
                        )
        ...
        for user in users_data:
            if (
                "noreply@" in (user.get("Email") or "").lower()
            ):  # ommiting the system users
                continue
            ...
            for permissions in user.get("PermissionSetAssignment", []):
                permissions_list, err = app.get_permissions_by_permissionset_id(
                    permissions.get("PermissionSetId")
                )
                if error:
                ...
                permissions_to_check = [
                    perm for perm in filtered_permissions if perm in unused_permissions
                ]
                if permissions_to_check:
                    users_permissions.update(permissions_to_check)
            if error:
                continue
            users_eventLog = pd.DataFrame()

            if not combined_event_logs.empty:
                if "USER_ID_DERIVED" in combined_event_logs.columns:
                    users_eventLog = combined_event_logs[
                        combined_event_logs["USER_ID_DERIVED"].str.contains(
                            user["Id"], na=False
                        )
                    ]
                elif "USER_ID" in combined_event_logs.columns:
                    users_eventLog = combined_event_logs[
                        combined_event_logs["USER_ID"].str.contains(
                            user["Id"], na=False
                        )
                    ]
            ...
            if not users_eventLog.empty:
                ...
                for events in event_type_list:
                    key_with_event = [
                        key
                        for key, value in unused_permissions.items()
                        if value == events
                    ]
                    for keyevent in key_with_event:
                        if keyevent in users_permissions:
                            users_permissions.discard(keyevent)
            ...
            user_response = {
                ...
                "Name": name,
                "UnusedPermissions": list(users_permissions),
                ...
                "EvaluatedTime": datetime.now(timezone.utc).strftime(
                    "%Y-%m-%d %H:%M:%S %Z"
                ),
                "UserAction": "",
                "ActionStatus": "",
            ...
            if len(user_response["UnusedPermissions"]) == 0:
                ...
                user_response["ValidationStatusNotes"] = (
                    "Unused permission(s) not present"
                )
                user_response["ComplianceStatus"] = "COMPLIANT"
                user_response["ComplianceStatusReason"] = (
                    f"The record is compliant because unused permissions are not present for the user - {user_response['ResourceName']}. "
                ...
                user_response["ComplianceStatusReason"] = (
                    ...
                    f"for the user - {user_response['ResourceName']}. Unused permissions can create potential vulnerabilities in your system, "
        ...
        file_url, error = self.upload_output_file(
            "SalesforceUnusedPermissions", standard_structure
        )
        if error:
            return self.upload_log_file_panic(error)
    ...
    def check_inputs(self):
        ...
        for input in required_inputs:
            if (
                not cowdictutils.is_valid_key(self.task_inputs.user_inputs, input)
                or self.task_inputs.user_inputs[input] == "<<MINIO_FILE_PATH>>"
            ):
    ...
    def upload_log_file(self, error_data) -> Tuple[str, dict]:
        ...
        file_url, error = self.upload_df_as_json_file_to_minio(
            df=pd.json_normalize(error_data), file_name="LogFile"
        )
        if error:
