...
class Task(cards.AbstractTask):
    ...
    def execute(self) -> dict:
        ...
        azure_users_list, error = azure_connector.list_azure_users()
        if error:
            return self.upload_log_file_panic({"Error": f"Error while getting azure users :: {error}"})
        
        azure_users_details = []
        for azure_users_data in azure_users_list :
            new_flattened_output = azure_connector.replace_empty_dicts_with_none(azure_users_data)

            flattened_output = azure_connector.flatten_json(new_flattened_output)
            azure_users_details.append(flattened_output)
        
        azure_users_df = pd.DataFrame(azure_users_details)
        file_url, error = self.upload_output_file(azure_users_df, "AzureUsersList")
        if error:
            return self.upload_log_file_panic(error)

        response = {
            "AzureUsersList": file_url
        }
