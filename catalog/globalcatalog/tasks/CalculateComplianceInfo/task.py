    ...
    def execute(self) -> dict:
        ...
        if self.task_inputs.user_inputs and isinstance(self.task_inputs.user_inputs,dict):
            ...
            for key,val in self.task_inputs.user_inputs.items():
                if isinstance(val,str) and val.startswith("http://"+os.getenv("MINIO_LOGIN_URL","cowstorage:9000")):
                    count +=1
                    evidence_df = pd.DataFrame()
                    evidence_data_path = val
                    file_bytes, err = self.download_file_from_minio(
                        file_url=evidence_data_path)
                    ...
                    if (file_extension == ".ndjson" or file_extension == ".json" or
                            file_extension == ".csv"):

                        message = file_bytes.decode('utf-8')
                        data = StringIO(message)
                        if file_extension == ".ndjson":
                            evidence_df = pd.read_json(data, lines=True,
                                            keep_default_dates=False, dtype=False)
        ...
        response = {
            "ComplianceStatus_": compliance_status,
            "CompliancePCT_": compliance_pct,
            "ComplianceWeight_":compliance_weight
        }
