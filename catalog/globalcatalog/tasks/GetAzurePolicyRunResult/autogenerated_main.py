...
for py_module in list(
    filter(
        lambda x: x.endswith(".py")
        and x != inspect.getfile(inspect.currentframe())
        and x != "autogenerated_main.py"
        and not x.startswith("."),
        os.listdir(path=dir_path),
    )
):
    filename, file_extension = os.path.splitext(os.path.basename(py_module))
    ...
    for name, obj in inspect.getmembers(module_type):
        if inspect.isclass(obj):
            if obj != cards.AbstractTask and issubclass(obj, cards.AbstractTask):
                ...
                data = json.loads(os.path.expandvars(json.dumps(data, cls=DateEncoder)))
                ...
                if not data.system_objects:
                    try:
                        default_system_objects = json.loads(
                            os.path.expandvars(
                        ...
                        data.meta_data = cowvo.MetaDataTemplate(
                            str(uuid.uuid4()),
                            str(uuid.uuid4()),
                            str(uuid.uuid4()),
                            str(uuid.uuid4()),
