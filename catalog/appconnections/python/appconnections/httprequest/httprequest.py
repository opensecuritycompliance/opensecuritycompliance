...
class HttpRequest:
    ...
    def make_api_call(self ,request_data):
        ...
        timeout = request_data.get('TimeOut', 60)  # Default timeout is 30 seconds
        retries = request_data.get('Retries', 1)  # Default retries is 3
        ...
        sucessful_statues = [200,201,202 , 204]
        if response.status_code in sucessful_statues :
    ...
    def send_request_with_retries(self ,
        ...
        attempt = 0
        while (attempt < retries) or (retries == 0 and attempt == 0):
            try:
                ...
                if (response.status_code in retry_on_status) or (response.status_code in [502 , 504, 429]): 
                    print(f"Received status {response.status_code}. Retrying... ({attempt + 1}/{retries})")
                    attempt += 1
                    sleep(2)  # Delay before retry
                else:
                    return response  # Return successful or non-retry status
    ...
    def generate_aws_iam_signature(self , service , region , api_url ,
    ...
    def generate_jwt_bearer(self):
        ...
        if error  :
            return None, "Error while generating authoraization."
        sucessful_statues = ['200','201','202' , '204']
        is_resp_valid = True if status_code in sucessful_statues else False
        if is_resp_valid:
    ...
    def generate_signature(self, method, service, host, 
                           region, canonical_uri, request_parameters, request_body, access_key, secret_key):
