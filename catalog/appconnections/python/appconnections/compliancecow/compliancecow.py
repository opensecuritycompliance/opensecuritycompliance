...
class ComplianceCow:
    ...
    UPDATE_EVIDENCE = '/v1/datahandler/'
    GET_APPLICATION_SCOPES = '/v2/configuration'
    CREATE_ASSESSMENT_RUN = '/v1/plan-instances'
    ...
    def fetch_auth_token(self):
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            ...
            return None, "Validation failed. Exception occurred while validating app."
    ...
    def fetch_evidence(self, assesment_run_id: str, control_id: str, evidence_id: str) -> Tuple[pd.DataFrame, dict, str]:
        ...
        if response.ok and response.status_code == http.HTTPStatus.OK:
            try:
                ...
                return None, None, f"Response is in an invalid format :: {e}"
        else:
            return None, None, f"Received error from response :: Status Code: {response.status_code}, Response: {response.text}"
    ...
    def get_plans(self):
        try:
            ...
            if error:
                return None, error
            api_endpoint_url = self.build_api_url(self.GET_PLANS)
            headers = {
                'Authorization': auth_token
            }
    ...
    def send_slack_notification(self, control_id, workflow_instance_id, user_mails):
        try:
            ...
            if err:
                return error
            if not hash:
                return f"Hash is empty. ControlID - {control_id}"
            
            app_url = self.app_url.rstrip("/")
            msg_body = f"{app_url}/ui/download/{hash}"
    ...
    def get_hash(self, control_id):  
        if not control_id:
            return None, "ControlID is empty"
